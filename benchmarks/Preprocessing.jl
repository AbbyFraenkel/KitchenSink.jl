...
function benchmark_solve_equation(dim::Int, num_elements::Int, polynomial_degree::Int)
    ...
    problem = KSPDEProblem{Float64,dim}(
        (D, u, x, idx) -> sum(-D[i][idx[i], :] ⋅ u for i in 1:dim),  # -Δu
...
function benchmark_solvers(A::AbstractMatrix{T}, b::AbstractVector{T}, solvers::Vector{S}, benchmark_func::Function = @benchmark) where {T<:Real, S<:AbstractKSLinearSolver}
