...
function create_test_problem_3d()
    KSProblem((x, y, z) -> x^2 + y^2 + z^2, (x, y, z) -> 0.0, domain_3d, KSCartesianCoordinates(domain_3d))
end
    ...
    @testset "create_mesh Function" begin
        @testset "Valid Cases" begin
            @testset "1D Mesh Creation" begin
                mesh_1d = create_mesh(domain_1d, num_elements_1d, polynomial_degree_1d, KSCartesianCoordinates(domain_1d))
                @test mesh_1d isa KSMesh{Float64, 1}
                @test length(mesh_1d.elements) == prod(num_elements_1d)
            end

            @testset "2D Mesh Creation" begin
                mesh_2d = create_mesh(domain_2d, num_elements_2d, polynomial_degree_2d, KSCartesianCoordinates(domain_2d))
                @test mesh_2d isa KSMesh{Float64, 2}
                @test length(mesh_2d.elements) == prod(num_elements_2d)
            end

            @testset "3D Mesh Creation" begin
                mesh_3d = create_mesh(domain_3d, num_elements_3d, polynomial_degree_3d, KSCartesianCoordinates(domain_3d))
        ...
        @testset "Invalid Cases" begin
            @testset "Invalid Domain Dimensionality" begin
                @test_throws AssertionError create_mesh(domain_1d, num_elements_2d, polynomial_degree_2d, KSCartesianCoordinates(domain_2d))
            end

            @testset "Non-Positive Number of Elements" begin
                @test_throws AssertionError create_mesh(domain_2d, [0, 2], polynomial_degree_2d, KSCartesianCoordinates(domain_2d))
            end

            @testset "Invalid Polynomial Degree" begin
                @test_throws AssertionError create_mesh(domain_2d, num_elements_2d, [0, 3], KSCartesianCoordinates(domain_2d))
            end
        end

        @testset "Edge Cases" begin
            @testset "Single Element Mesh" begin
                mesh_single = create_mesh(domain_1d, [1], polynomial_degree_1d, KSCartesianCoordinates(domain_1d))
                @test length(mesh_single.elements) == 1
            end

            @testset "High Polynomial Degree" begin
                mesh_high_degree = create_mesh(domain_1d, num_elements_1d, [10], KSCartesianCoordinates(domain_1d))
    ...
    @testset "assemble_local_system_matrix Function" begin
        @testset "Valid Cases" begin
            @testset "2D Local System Matrix Assembly" begin
                ...
                A_local_2d = assemble_local_system_matrix(element_2d, mesh_2d, KSCartesianCoordinates(domain_2d))
                @test size(A_local_2d) == (prod(polynomial_degree_2d .+ 1), prod(polynomial_degree_2d .+ 1))
    ...
    @testset "inner_product Function" begin
        @testset "Valid Cases" begin
            @testset "2D Inner Product Calculation" begin
                mesh_2d = create_mesh(domain_2d, num_elements_2d, polynomial_degree_2d, KSCartesianCoordinates(domain_2d))
                element_2d = mesh_2d.elements[1]
                std_elem = create_or_get_standard_element(element_2d.polynomial_degree, element_2d.level)
    ...
    @testset "create_or_get_standard_element Function" begin
        @testset "Valid Cases" begin
            @testset "Create and Cache 2D Standard Element" begin
                ...
                @test std_elem isa StandardElement{Float64, 2}
