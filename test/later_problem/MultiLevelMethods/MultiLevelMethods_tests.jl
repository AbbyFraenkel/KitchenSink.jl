...
@testset "MultiLevelMethods" begin
    @testset "create_mesh_hierarchy" begin
        ...
        base_mesh = CommonMethods.create_mesh(
        ...
        @testset "Valid input" begin
            ...
            @test all(
                i -> length(hierarchy[i].elements) > length(hierarchy[i - 1].elements),
                2:3
            )
        end
    ...
    @testset "coarsen_mesh" begin
        ...
        polynomial_degree = 2
        coord_system = KSCartesianCoordinates(domain)
        mesh = CommonMethods.create_mesh(
            domain, coord_system, num_elements, polynomial_degree)

        @testset "Valid input" begin
            coarse_mesh = coarsen_mesh(mesh)
            @test coarse_mesh isa KSMesh{Float64, 1}
            @test length(coarse_mesh.cells) == 2
        end

        @testset "Invalid input" begin
            single_element_mesh = CommonMethods.create_mesh(
                domain, coord_system, [1], polynomial_degree)
    ...
    @testset "merge_elements" begin
        ...
        domain = [(0.0, 1.0)]
        coord_system = KSCartesianCoordinates(domain)
        el1 = CommonMethods.create_elements(
            [KSPoint([0.0]), KSPoint([0.5])],
            [1],
            2,
            coord_system
        )[1]
        el2 = CommonMethods.create_elements(
            [KSPoint([0.5]), KSPoint([1.0])],
            [1],
            2,
            coord_system
        )[1]
        ...
        @test merged_element isa KSCell{Float64, 1}
        @test length(merged_element.points) == 3
        @test merged_element.polynomial_degree ==
              max(el1.polynomial_degree, el2.polynomial_degree)
    end
    ...
    @testset "refine_mesh_uniformly" begin
        ...
        @test length(refined_mesh.cells) == 2 * length(mesh.cells)
    end
    ...
    @testset "refine_marked_elements" begin
        ...
        @test refined_mesh isa KSMesh{Float64, 1}
        @test length(refined_mesh.cells) == length(mesh.cells) + 2
    end
    ...
    @testset "adjust_finer_level" begin
        ...
        polynomial_degree = 2
        coord_system = KSCartesianCoordinates(domain)
        coarse_mesh = CommonMethods.create_mesh(
            domain, coord_system, num_elements, polynomial_degree)
        ...
        @test length(adjusted_fine_mesh.cells) == length(fine_mesh.cells)
    end
    ...
    @testset "geometric_multigrid" begin
        ...
        @testset "Invalid cycle type" begin
            @test_throws ErrorException geometric_multigrid(
                A,
                b,
                hierarchy,
                cycle_type = :invalid
    ...
    @testset "Helper functions" begin
        ...
        @testset "estimate_convergence_factor" begin
            ...
            u_exact_vec = [u_exact([x])
                           for
                           x in LinRange(0, 1, CommonMethods.total_dofs(hierarchy[end]))]
