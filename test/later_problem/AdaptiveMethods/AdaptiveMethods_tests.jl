...
function create_mock_element(id::Int, dimension::Int, polynomial_degree::Int)
    points = [KSPoint([i / polynomial_degree for _ in 1:dimension])
              for i in 0:polynomial_degree]
    basis_functions = [KSBasisFunction(i, x -> sum(x .^ i))
                       for i in 1:(polynomial_degree + 1)]
    collocation_points = [KSPoint([i / (polynomial_degree + 1) for _ in 1:dimension])
                          for
                          i in 1:(polynomial_degree + 1)]
    diff_matrices = [rand(polynomial_degree + 1, polynomial_degree + 1)
...
function create_mock_mesh(num_elements::Int, dimension::Int, polynomial_degree::Int)
    elements = [create_mock_element(i, dimension, polynomial_degree)
                for i in 1:num_elements]
    tensor_product_masks = [trues(tuple(fill(polynomial_degree + 1, dimension)...))
                            for _ in 1:num_elements]
    location_matrices = [Dict(i => i for i in 1:((polynomial_degree + 1)^dimension))
                         for _ in 1:num_elements]
...
@testset "AdaptiveMethods" begin
    @testset "h_refine" begin
        ...
        @test all(
            e.polynomial_degree == element.polynomial_degree for e in refined_elements
        )
    end
    ...
    @testset "p_refine" begin
        ...
        @test length(refined_element.collocation_points) >
              length(element.collocation_points)
    ...
    @testset "superconvergence_refinement" begin
        ...
        AdaptiveMethods.estimate_error_and_smoothness(
        ::KSCell,
        ::Vector{Float64},
        ::MockProblem
) = (1.0, 0.1)

        refined_elements = AdaptiveMethods.superconvergence_refinement(
    ...
    @testset "adapt_mesh_superposition" begin
        ...
        solution = rand(sum(length(e.collocation_points) for e in mesh.cells))
        problem = MockProblem()

        # Mock the superconvergence_refinement function
        AdaptiveMethods.superconvergence_refinement(
        ::KSCell,
        ::Vector{Float64},
        ::MockProblem
) = [create_mock_element(1, 2, 4)]

        adapted_mesh = AdaptiveMethods.adapt_mesh_superposition(
            mesh, solution, problem, 1.0)

        @test length(adapted_mesh.cells) == length(mesh.cells)
        @test all(e.polynomial_degree == 4 for e in adapted_mesh.cells)
    end
    ...
    @testset "estimate_error_and_smoothness" begin
        ...
        CommonMethods.estimate_error(::KSCell, ::Vector{Float64}, ::MockProblem) = 1.0
        CommonMethods.estimate_smoothness(::KSCell, ::Vector{Float64}) = 0.5

        error, smoothness = AdaptiveMethods.estimate_error_and_smoothness(
            element, solution, problem)
    ...
    @testset "compute_error_indicator" begin
        ...
        error, smoothness = AdaptiveMethods.compute_error_indicator(
            element, solution, problem)
...
@testset "AdaptiveMethods Tests" begin
    ...
    @testset "select_subelement_nodes" begin
        mesh = create_test_mesh()
        element = mesh.cells[1]
        mid_points = [(node1.coordinates .+ node2.coordinates) ./ 2
                      for node1 in element.points
                      for node2 in element.points if node1 != node2]
    ...
    @testset "hp_refine_superposition" begin
        ...
        @test all(e -> e isa KSCell, refined_elements)
    end
