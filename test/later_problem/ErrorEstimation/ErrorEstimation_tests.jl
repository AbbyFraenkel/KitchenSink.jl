...
using KitchenSink.KSTypes,
      KitchenSink.CoordinateSystems, KitchenSink.SpectralMethods, KitchenSink.CommonMethods
...
@testset "ErrorEstimation Tests 2.0" begin
    ...
    function create_test_mesh()
        ...
        num_elements = [2, 2]
        polynomial_degree = 2
        return CommonMethods.create_mesh(
            domain,
            num_elements,
            polynomial_degree,
            coord_system
    ...
    @testset "compute_residual" begin
        ...
        constant_solution = ones(length(element.collocation_points))
        constant_residual = ErrorEstimation.compute_residual(
            element, constant_solution, problem)
    ...
    @testset "compute_error_indicator" begin
        ...
        error, smoothness = ErrorEstimation.compute_error_indicator(
            element, solution, problem)
        @test error isa Real
        ...
        constant_solution = ones(length(element.collocation_points))
        constant_error, constant_smoothness = ErrorEstimation.compute_error_indicator(
            element, constant_solution, problem)
        @test constant_error > error  # Constant solution should have larger error
        ...
        # Test with different types
        solution_f32 = Float32.(solution)
        error_f32, smoothness_f32 = ErrorEstimation.compute_error_indicator(
            element, solution_f32, problem)
        @test error_f32 isa Float32
    ...
    @testset "compute_smoothness_indicator" begin
        ...
        constant_solution = ones(length(element.collocation_points))
        constant_smoothness = ErrorEstimation.compute_smoothness_indicator(
            element, constant_solution)
        @test constant_smoothness > smoothness

        # Test with a highly oscillatory solution (should be less smooth)
        oscillatory_solution = [sin(10π * p.coordinates[1]) * sin(10π * p.coordinates[2])
                                for
                                p in element.collocation_points]
        oscillatory_smoothness = ErrorEstimation.compute_smoothness_indicator(
            element, oscillatory_solution)
    ...
    @testset "apply_differential_operator" begin
        ...
        # Test with a constant solution
        constant_solution = ones(length(element.collocation_points))
        constant_result = ErrorEstimation.apply_differential_operator(
            constant_solution, element, problem)
        @test all(isapprox.(constant_result, 1.0, atol = 1e-10))

        # Test with different types
        solution_f32 = Float32.(solution)
        result_f32 = ErrorEstimation.apply_differential_operator(
            solution_f32, element, problem)
    ...
    @testset "estimate_smoothness" begin
        ...
        constant_smoothness = ErrorEstimation.estimate_smoothness(
            element, constant_solution)
        @test constant_smoothness > smoothness
        ...
        oscillatory_solution = [sin(10π * p.coordinates[1]) * sin(10π * p.coordinates[2])
                                for
                                p in element.collocation_points]
        oscillatory_smoothness = ErrorEstimation.estimate_smoothness(
            element, oscillatory_solution)
        @test oscillatory_smoothness < smoothness

        # Test error handling
        @test_throws DimensionMismatch ErrorEstimation.estimate_smoothness(
            element,
            ones(CommonMethods.element_dofs(element) + 1)
        )
    ...
    @testset "compute_expansion_coefficients" begin
        ...
        # Test reconstruction
        reconstructed_solution = [sum(
                                      c * bf.function_handle(p.coordinates)
                                  for
                                  (c, bf) in zip(coeffs, element.basis_functions)
                                  ) for p in element.collocation_points]
        @test isapprox(solution, reconstructed_solution, rtol = 1e-5)

        # Test error handling
        @test_throws ArgumentError ErrorEstimation.compute_expansion_coefficients(
            element,
            solution,
            lambda = -1.0
        )
        @test_throws DimensionMismatch ErrorEstimation.compute_expansion_coefficients(
            element,
            ones(length(solution) + 1)
        )
        ...
        element_singular.basis_functions = [KSBasisFunction(1, x -> 1.0)
                                            for _ in 1:length(element_singular.basis_functions)]
        @test_throws LinearAlgebra.SingularException ErrorEstimation.compute_expansion_coefficients(
            element_singular,
            solution
        )
    end
