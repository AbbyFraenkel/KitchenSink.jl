...
using KitchenSink.Preprocessingw
...
@testset "Preprocessing Tests" begin
	...
	@testset "preprocess_mesh" begin
		@testset "Valid cases" begin
			...
			@test length(mesh.cells) >= prod(num_elements)
		...
		@testset "Invalid cases" begin
			...
			@test_throws DimensionMismatch preprocess_mesh(
				problem,
				[(1.0, 0.0), (0.0, 1.0)],
				KSCartesianCoordinates(2),
				[2, 2],
				3,
				3,
				1e-6,
	...
	@testset "generate_initial_mesh" begin
		@testset "Valid cases" begin
			...
			mesh = generate_initial_mesh(
				domain, coord_system, num_elements, polynomial_degree)
			@test mesh isa KSMesh
			@test length(mesh.cells) == prod(num_elements)
			@test all(el.polynomial_degree == polynomial_degree for el in mesh.cells)
		end
		...
		@testset "Edge cases" begin
			...
			@test mesh isa KSMesh
			@test length(mesh.cells) == 1
			@test mesh.cells[1].polynomial_degree == 1
		end
		...
		@testset "Invalid cases" begin
			...
			@test_throws DimensionMismatch generate_initial_mesh(
				domain,
				KSCartesianCoordinates(3),
				[2, 2],
				3,
			)
			@test_throws DimensionMismatch generate_initial_mesh(
	...
	@testset "refine_mesh" begin
		@testset "Valid cases" begin
			...
			@test length(refined_mesh.cells) > length(mesh.cells)
		end
	...
	@testset "estimate_mesh_error" begin
		@testset "Valid cases" begin
			...
			@test length(error_estimates) == length(mesh.cells)
		...
		@testset "Invalid cases" begin
			...
			@test_throws ArgumentError estimate_mesh_error(
				mesh,
				KSPDEProblem(nothing, nothing, nothing, nothing, nothing),
			)
		end
	end

	@testset "create_OCFC_discretization" begin
		@testset "Valid cases" begin
			problem = create_test_problem()
			mesh = generate_initial_mesh(
				[(0.0, 1.0), (0.0, 1.0)],
			...
			A = create_OCFC_discretization(mesh, problem, max_derivative_order)
		...
		@testset "Complex cases" begin
			...
			A = create_OCFC_discretization(mesh, complex_problem, max_derivative_order)
			@test size(A, 1) == size(A, 2)
			@test size(A, 1) >= 0
			@test issparse(A)
		end
		...
		@testset "Invalid cases" begin
			...
			@test_throws ArgumentError create_OCFC_discretization(mesh, problem, -1)
			@test_throws ArgumentError create_OCFC_discretization(mesh, problem, 0)
		end
	end
end
