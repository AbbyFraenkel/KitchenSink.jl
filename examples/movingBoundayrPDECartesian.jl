...
function test_stefan_problem()
    ...
    results = solve_moving_boundary_pde(domain, tspan, pde_function, bc_function, ic_function, boundary_function)
    ...
    plot(results["time"], results["boundary"], xlabel="Time", ylabel="Boundary Position", title="Stefan Problem: Numerical vs Analytical Boundary Movement")
    plot!(results["time"], analytical_s.(results["time"]), label="Analytical Boundary", linestyle=:dash)
    println("Stefan Problem: Max Error in Boundary Position = ", maximum(abs.(results["boundary"] - analytical_s.(results["time"]))))

    # Plot results for temperature profile
    u_num = results["solution"][end]
    x_vals = linspace(domain[1], results["boundary"][end], length(u_num))
    plot(x_vals, u_num, xlabel="x", ylabel="Temperature", title="Temperature Profile at Final Time", label="Numerical Solution")
...
function test_semi_infinite_heat_conduction()
    ...
    # Plot results for boundary movement
    plot(results["time"], results["boundary"], xlabel="Time", ylabel="Boundary Position", title="Semi-Infinite Conduction: Numerical vs Analytical Boundary Movement")
    plot!(results["time"], analytical_s.(results["time"]), label="Analytical Boundary", linestyle=:dash)
    println("Semi-Infinite Conduction: Max Error in Boundary Position = ", maximum(abs.(results["boundary"] - analytical_s.(results["time"]))))
...
function test_solidification()
    ...
    # Plot results for boundary movement
    plot(results["time"], results["boundary"], xlabel="Time", ylabel="Boundary Position", title="Solidification: Numerical vs Analytical Boundary Movement")
    plot!(results["time"], analytical_s.(results["time"]), label="Analytical Boundary", linestyle=:dash)
    println("Solidification: Max Error in Boundary Position = ", maximum(abs.(results["boundary"] - analytical_s.(results["time"]))))
