...
"""
function create_mesh_hierarchy(base_mesh::KSMesh{T,N}, max_levels::Int) where {T<:Real,N}
...
function coarsen_mesh(mesh::KSMesh{T,N}) where {T<:Real,N}
    length(mesh.elements) > 1 || throw(ArgumentError("Mesh must have more than one element to coarsen"))
    ...
    return KSMesh(
        ...
        standard_elements = Dict{Tuple{Int, Int}, StandardElement{T, N}}(),  # Initialize empty standard element cache
        global_error_estimate = mesh.global_error_estimate
    )
end

"""
    merge_elements(el1::KSElement{T,N}, el2::KSElement{T,N}) where {T<:Real,N} -> KSElement{T,N}

Merge two elements into a single, coarser element.

# Arguments
- `el1::KSElement{T,N}`: The first element to merge
- `el2::KSElement{T,N}`: The second element to merge

# Returns
- `KSElement{T,N}`: The merged element
"""
function merge_elements(el1::KSElement{T,N}, el2::KSElement{T,N}) where {T<:Real,N}
    ...
    # Retrieve or create the StandardElement for the merged element
    new_degree = max(el1.polynomial_degree, el2.polynomial_degree)
    std_elem = SpectralMethods.create_standard_element(new_degree)
    transformed_std_elem = SpectralMethods.transform_standard_element(std_elem, (el1.domain..., el2.domain...), new_level)
...
function refine_mesh_uniformly(mesh::KSMesh{T,N}) where {T<:Real,N}
    ...
    new_standard_elements = Dict{Tuple{Int, Int}, StandardElement{T, N}}()

    # Ensure StandardElement cache is updated for the refined elements
    for el in new_elements
        key = (el.level, el.polynomial_degree)
        if !haskey(new_standard_elements, key)
            std_elem = SpectralMethods.create_standard_element(el.polynomial_degree)
...
"""
function refine_mesh_hierarchy(hierarchy::Vector{KSMesh{T,N}}, level::Int, marked_elements::AbstractVector{Int}) where {T<:Real,N}
...
- `KSMesh{T,N}`: The refined mesh
"""
function refine_marked_elements(mesh::KSMesh{T,N}, marked_elements::AbstractVector{Int}) where {T<:Real,N}
    ...
    for (i, el) in enumerate(mesh.elements)
        if i in marked_elements
            ...
            # Update StandardElement cache for refined elements
            for sub_el in refined_elements
                key = (sub_el.level, sub_el.polynomial_degree)
                if !haskey(new_standard_elements, key)
                    std_elem = SpectralMethods.create_standard_element(sub_el.polynomial_degree)
                    new_standard_elements[key] = std_elem
                end
            end
        else
...
"""
function adjust_finer_level(coarse_mesh::KSMesh{T,N}, fine_mesh::KSMesh{T,N}) where {T<:Real,N}
    new_elements = Vector{KSElement{T,N}}()
...
- `Vector{T}`: The updated solution vector
"""
function w_cycle(hierarchy::Vector{KSMesh{T,N}}, f::Function, u::Vector{T}, level::Int) where {T<:Real,N}
    if level == 1
        return solve_coarse_problem(hierarchy[1], f)
    else
        u = smooth(hierarchy[level], f, u)
...
"""
function geometric_multigrid(A::AbstractMatrix, b::AbstractVector, mesh_hierarchy::Vector{KSMesh{T,N}};
                             cycle_type::Symbol=:v, max_iterations::Int=100, tolerance::Float64=1e-8) where {T<:Real,N}
    x = zeros(T, length(b))
    r = b - A * x
...
"""
function algebraic_multigrid(A::AbstractMatrix, b::AbstractVector; max_iterations::Int=100, tolerance::Float64=1e-8)
    ml = AlgebraicMultigrid.ruge_stuben(A)
    x, convergence_history = AlgebraicMultigrid.solve(ml, b, maxiter=max_iterations, tol=tolerance, log=true)
...
"""
function compute_residual(mesh::KSMesh{T,N}, f::Function, u::Vector{T}) where {T<:Real,N}
    A = CommonMethods.assemble_system_matrix(mesh)
    b = CommonMethods.assemble_rhs_vector(mesh, f)
    return b - A * u
end
...
function validate_mesh_hierarchy(hierarchy::Vector{KSMesh{T,N}}) where {T<:Real,N}
    ...
    for i in 2:length(hierarchy)
        ...
        if length(hierarchy[i].elements) <= length(hierarchy[i-1].elements)
            throw(ArgumentError("Each level in the hierarchy should have more elements than the previous one"))
        end
    end
end
...
function compute_mesh_complexity(hierarchy::Vector{KSMesh{T,N}}) where {T<:Real,N}
    ...
    total_dofs = sum(sum(CommonMethods.element_dofs(el) for el in mesh.elements) for mesh in hierarchy)
...
"""
function estimate_convergence_factor(hierarchy::Vector{KSMesh{T,N}}, f::Function, u_exact::Vector{T}, num_iterations::Int) where {T<:Real,N}
    validate_mesh_hierarchy(hierarchy)
