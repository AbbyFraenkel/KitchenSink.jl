...
export prolongate!,
	restrict!, interpolate_between_meshes!, find_parent_element, find_child_elements
...
"""
function prolongate!(
	fine_solution::AbstractVector{T},
	coarse_solution::AbstractVector{T},
	coarse_mesh::KSMesh{T, N},
	fine_mesh::KSMesh{T, N},
) where {T <: Real, N}
	for (i, fine_element) in enumerate(fine_mesh.cells)
		coarse_element = find_parent_element(fine_element, coarse_mesh)
		if coarse_element !== nothing
			std_elem = SpectralMethods.get_or_get_or_create_standard_cell(
				coarse_element.polynomial_degree
			)
			transformed_elem = SpectralMethods.transform_standard_element(
				std_elem,
				coarse_element.level,
				coarse_element.polynomial_degree,
			)
			...
			fine_local_solution = interpolate_element_with_standard(
				fine_element,
				coarse_element,
				coarse_local_solution,
				transformed_elem,
			)
...
function restrict!(
	...
	for (i, coarse_element) in enumerate(coarse_mesh.cells)
		...
		if !isempty(fine_elements)
			...
			for fine_element in fine_elements
				std_elem = SpectralMethods.get_or_get_or_create_standard_cell(
					fine_element.polynomial_degree
				)
				transformed_elem = SpectralMethods.transform_standard_element(
					std_elem,
					fine_element.physical_domain,
					fine_element.level,
				)
				...
				coarse_local_solution .+= restrict_element_with_standard(
					fine_element,
					coarse_element,
					fine_local_solution,
					transformed_elem,
...
"""
function interpolate_between_meshes!(
	target_solution::AbstractVector{T},
	source_solution::AbstractVector{T},
	source_mesh::KSMesh{T, N},
	target_mesh::KSMesh{T, N},
) where {T <: Real, N}
	for (i, target_element) in enumerate(target_mesh.cells)
		source_element = find_containing_element(
			target_element.collocation_points[1], source_mesh)
		if source_element !== nothing
			source_local_solution = source_solution[get_active_indices(
				source_mesh, source_element)]
			target_local_solution = interpolate_element(
				source_element, target_element, source_local_solution)
			target_solution[get_active_indices(target_mesh, target_element)] .=
				target_local_solution
		end
	end
...
"""
	find_parent_element(fine_element::KSCell{T,N}, coarse_mesh::KSMesh{T,N}) where {T<:Real,N}

Find the parent element of a fine element in the coarse mesh.

# Arguments
- `fine_element::KSCell{T,N}`: The fine element.
- `coarse_mesh::KSMesh{T,N}`: The coarse mesh.

# Returns
- `Union{KSCell{T,N}, Nothing}`: The parent element if found, nothing otherwise.
"""
function find_parent_element(
	fine_element::KSCell{T, N},
	coarse_mesh::KSMesh{T, N},
) where {T <: Real, N}
	parent_coords = fine_element.get_parent_coords()
	for coarse_element in coarse_mesh.cells
...
"""
	find_child_elements(coarse_element::KSCell{T,N}, fine_mesh::KSMesh{T,N}) where {T<:Real,N}

Find the child elements of a coarse element in the fine mesh.

# Arguments
- `coarse_element::KSCell{T,N}`: The coarse element.
- `fine_mesh::KSMesh{T,N}`: The fine mesh.

# Returns
- `Vector{KSCell{T,N}}`: The child elements.
"""
function find_child_elements(
	coarse_element::KSCell{T, N},
	fine_mesh::KSMesh{T, N},
) where {T <: Real, N}
	...
	return [
		fine_element
		for
		fine_element in fine_mesh.cells if fine_element.matches_coords(child_coords)
	]
end
...
# Returns
- `Union{KSCell{T,N}, Nothing}`: The containing element if found, nothing otherwise.
"""
function find_containing_element(
...
"""
	interpolate_element(source_element::KSCell{T,N}, target_element::KSCell{T,N},
						source_solution::AbstractVector{T}) where {T<:Real,N}

Interpolate the solution from a source element to a target element.

# Arguments
- `source_element::KSCell{T,N}`: The source element.
- `target_element::KSCell{T,N}`: The target element.
...
"""
function interpolate_element(
	source_element::KSCell{T, N},
	target_element::KSCell{T, N},
	source_solution::AbstractVector{T},
) where {T <: Real, N}
	std_elem = SpectralMethods.get_or_get_or_create_standard_cell(
		source_element.polynomial_degree
	)
	transformed_elem = SpectralMethods.transform_standard_element(
		std_elem,
		source_element.physical_domain,
		source_element.level,
	)
	...
	for (i, point) in enumerate(target_element.collocation_points)
		target_solution[i] = interpolate_point(
			point, source_element, source_solution, transformed_elem)
	end
	return target_solution
end

"""
	interpolate_point(point::AbstractVector{T}, element::KSCell{T,N},
					  solution::AbstractVector{T}, std_elem::StandardKSCell{T,N}) where {T<:Real,N}

Interpolate the solution at a specific point within an element.

# Arguments
- `point::AbstractVector{T}`: The point coordinates.
- `element::KSCell{T,N}`: The element containing the solution.
- `solution::AbstractVector{T}`: The solution on the element.
- `std_elem::StandardKSCell{T,N}`: The standard element containing precomputed data.
...
"""
	is_parent(coarse_element::KSCell{T,N}, fine_element::KSCell{T,N}) where {T<:Real,N}

Check if a coarse element is the parent of a fine element.

# Arguments
- `coarse_element::KSCell{T,N}`: The potential parent element.
- `fine_element::KSCell{T,N}`: The potential child element.

# Returns
- `Bool`: True if coarse_element is the parent of fine_element, false otherwise.
"""
function is_parent(
	coarse_element::KSCell{T, N}, fine_element::KSCell{T, N}) where {T <: Real, N}
	return all(
		is_point_in_element(point, coarse_element)
		for
		point in fine_element.collocation_points
	)
end
...
"""
function validate_mesh_compatibility(
	mesh1::KSMesh{T, N},
	mesh2::KSMesh{T, N},
) where {T <: Real, N}
...
"""
	get_active_indices(mesh::KSMesh{T,N}, element::KSCell{T,N}) where {T<:Real,N}

Helper function to get active indices for an element in a mesh.
"""
function get_active_indices(mesh::KSMesh{T, N}, element::KSCell{T, N}) where {T <: Real, N}
	element_index = findfirst(e -> e.id == element.id, mesh.cells)
	return collect(values(mesh.location_matrices[element_index]))
end

"""
	compute_interpolation_weights(point::AbstractVector{T}, element::KSCell{T,N}) where {T<:Real,N}
...
- `element::KSCell{T,N}`: The element containing the point.

# Returns
- `AbstractVector{T}`: The interpolation weights.
"""
function compute_interpolation_weights(
	point::AbstractVector{T},
	element::KSCell{T, N},
) where {T <: Real, N}
...
"""
	restrict_element(fine_element::KSCell{T,N}, coarse_element::KSCell{T,N},
