...
export inner_product, total_dofs, element_dofs, create_ocfc_discretization
export update_location_matrix!, create_or_get_standard_element
export deactivate_internal_boundary!, cache_scaled_element
# Thread-safe cache for StandardKSCell instances
# global STANDARD_ELEMENT_CACHE = Dict{Tuple{NTuple{N, Int} where N, Int}, StandardKSCell{T, N} where {T <: Real, N}}()
...
function create_mesh(
	domain::Union{AbstractVector{<:Tuple{T, T}}, Tuple{Vararg{Tuple{T, T}}}},
	...
	if use_caching
		mesh = create_mesh_with_caching(
			domain, num_elements, polynomial_degree, coord_system)
	else
...
"""
function create_mesh_with_caching(
	domain::Union{AbstractVector{<:Tuple{T, T}}, Tuple{Vararg{Tuple{T, T}}}},
	num_elements::AbstractVector{<:Integer},
	polynomial_degree::AbstractVector{<:Integer},
	coord_system::AbstractKSCoordinateSystem,
	...
	# Create base mesh using create_ocfc_mesh
	mesh = SpectralMethods.create_ocfc_mesh(
		coord_system,
		num_elements,
		polynomial_degree,
	...
	unique_configs = unique(zip(polynomial_degree, [1 for _ in 1:length(mesh.cells)])) # Example level=1
...
Creates a StandardKSCell or retrieves it from cache in a thread-safe manner.
"""
function create_or_get_standard_element(polynomial_degree::Tuple{Vararg{Int}}, level::Int)
	...
	lock(STANDARD_ELEMENT_LOCK) do
		if !haskey(STANDARD_ELEMENT_CACHE, key)
			STANDARD_ELEMENT_CACHE[key] = get_or_create_standard_cell(
				polynomial_degree, level)
		end
	end
...
"""
	get_or_create_standard_cell(polynomial_degree::Tuple{Vararg{Int}}, level::Int)

Creates a StandardKSCell for the given polynomial degree and refinement level.
Uses SpectralMethods functions for node and matrix generation.
"""
function get_or_create_standard_cell(polynomial_degree::Tuple{Vararg{Int}}, level::Int)
	T = Float64  # You might want to make this parametric

	# Create Legendre nodes and weights using SpectralMethods
	nodes_with_boundary, weights_with_boundary, collocation_points, quad_weights = SpectralMethods.gauss_legendre_with_boundary_nd(
		maximum(polynomial_degree),
		length(polynomial_degree),
	)

	# Create differentiation matrices using SpectralMethods
	diff_matrices = [
		SpectralMethods.derivative_matrix!(getindex.(nodes_with_boundary, i))
		for
		i in 1:length(polynomial_degree)
	]

	# Create quadrature matrices using SpectralMethods
	quad_matrices = [
		SpectralMethods.quadrature_matrix!(getindex.(quad_weights, i))
		for
		i in 1:length(polynomial_degree)
	]

	return StandardKSCell{T, length(polynomial_degree)}(
		nodes_with_boundary,
		collocation_points,
		quad_weights,
		diff_matrices,
...
from `StandardKSCell`. Uses parallel processing for improved performance.
"""
function assemble_system_matrix(
	...
	local_matrices = Vector{Tuple{Vector{Int}, AbstractMatrix{T}}}(
		undef, length(mesh.cells))

	for i in eachindex(mesh.cells)
		indices = get_active_indices(mesh.cells[i], mesh)
		A_local = assemble_local_system_matrix(mesh.cells[i], mesh, coord_system)
...
"""
	assemble_local_system_matrix(element::KSCell{T, N}, mesh::KSMesh{T, N}, coord_system::AbstractKSCoordinateSystem) where {T <: Real, N}

Assembles the local system matrix for a given element using precomputed differentiation matrices
from `StandardKSCell` and taking into account the coordinate system.
"""
function assemble_local_system_matrix(
	element::KSCell{T, N},
	mesh::KSMesh{T, N},
	coord_system::AbstractKSCoordinateSystem,
) where {T <: Real, N}
	...
	# jacobian = compute_jacobian(element.nodes[1], coord_system)
...
function assemble_rhs_vector(
	...
	local_vectors = Vector{Tuple{Vector{Int}, Vector{T}}}(undef, length(mesh.cells))

	for i in eachindex(mesh.cells)
		indices = get_active_indices(mesh.cells[i], mesh)
		b_local = assemble_local_rhs_vector(mesh.cells[i], mesh, f, coord_system)
...
"""
	assemble_local_rhs_vector(element::KSCell{T, N}, mesh::KSMesh{T, N}, f::Function, coord_system::AbstractKSCoordinateSystem) where {T <: Real, N}

Assembles the local right-hand side (RHS) vector for a given element using precomputed basis functions
and collocation points from `StandardKSCell`, taking into account the coordinate system.
"""
function assemble_local_rhs_vector(
	element::KSCell{T, N},
	mesh::KSMesh{T, N},
	f::Function,
	coord_system::AbstractKSCoordinateSystem,
	...
	physical_points = [
		map_from_reference_cell(coord, coord_system)
		for
		coord in std_elem.collocation_points
	]

	for (i, coord) in enumerate(physical_points)
		b_local[i] =
			f(coord...) *
			inner_product(std_elem.collocation_weights, ones(T, n), element, mesh)
	end
...
"""
	get_active_indices(element::KSCell{T, N}, mesh::KSMesh{T, N}) where {T <: Real, N}

Retrieves the global indices of active degrees of freedom for the specified element.
"""
function get_active_indices(element::KSCell{T, N}, mesh::KSMesh{T, N}) where {T <: Real, N}
	element_id = findfirst(el -> el.id == element.id, mesh.cells)
...
"""
	inner_product(f::AbstractVector{T}, g::AbstractVector{T}, element::KSCell{T, N}, mesh::KSMesh{T, N}) where {T <: Real, N}
...
function inner_product(
	...
	if length(f) != length(g) || length(f) != length(std_elem.collocation_points)
		throw(
			DimensionMismatch("Input vectors must match the number of collocation points.")
		)
	end
...
"""
	create_ocfc_discretization(mesh::KSMesh{T, N}, problem::AbstractKSProblem) where {T <: Real, N}

Creates the orthogonal collocation on finitecellsdiscretization for a given mesh and problem.
"""
...
"""
function update_location_matrix!(
	location_matrix::Dict{Int, Int},
	active_indices::AbstractVector{Int},
	global_index::Int,
)
...
"""
function deactivate_internal_boundary!(
...
"""
	cache_scaled_element(std_elem::StandardKSCell{T, N}, element_size::NTuple{N, T}) where {T <: Real, N}

Caches the scaled version of a StandardKSCell to avoid redundant scaling computations.
"""
function cache_scaled_element(
	std_elem::StandardKSCell{T, N},
	element_size::NTuple{N, T},
) where {T <: Real, N}
...
"""
	scale_standard_element(std_elem::StandardKSCell{T, N}, element_size::NTuple{N, T}) where {T <: Real, N}

Scales a StandardKSCell's differentiation matrices and collocation points to match a given element's size.
"""
function get_or_create_standard_cell(polynomial_degree::Tuple{Vararg{Int}}, level::Int)
	T = Float64  # You might want to make this parametric
...
"""
	element_dofs(element::KSCell{T, N}, mesh::KSMesh{T, N}) where {T <: Real, N}

Get the number of degrees of freedom (DOFs) associated with a specific element.
"""
function element_dofs(element::KSCell{T, N}, mesh::KSMesh{T, N}) where {T <: Real, N}
	element_id = findfirst(isequal(element), mesh.cells)
