...
function decompose_domain(mesh::KSMesh, num_subdomains::Int)
    ...
    nx = floor(Int, sqrt(length(mesh.cells)))
    ny = ceil(Int, length(mesh.cells) / nx)
    ...
    for i in 1:nx_sub:nx, j in 1:ny_sub:ny
        ...
        for ei in i:min(i + nx_sub - 1, nx), ej in j:min(j + ny_sub - 1, ny)
            ...
            push!(elements, mesh.cells[element_idx])
        ...
        submesh = KSMesh(
            elements,
            [m
             for (idx, m) in enumerate(mesh.tensor_product_masks)
             if
             mesh.cells[idx] in elements],
            [Dict(k => global_to_local[v] for (k, v) in m)
             for
             (idx, m) in enumerate(mesh.location_matrices) if mesh.cells[idx] in elements],
...
function create_interface_conditions(subdomains::Vector{Subdomain})
    ...
    for i in 1:length(subdomains)
        for j in (i + 1):length(subdomains)
            shared_dofs = intersect(
                keys(subdomains[i].global_to_local),
                keys(subdomains[j].global_to_local)
            )
...
function schwarz_method(
    ...
    solutions = [zeros(CommonMethods.total_dofs(subdomain.mesh))
    ...
    for _ in 1:max_iterations
        ...
        for (i, subdomain) in enumerate(subdomains)
            ...
            local_solution, _ = ProblemTypes.solve_problem(
                problem, subdomain.mesh, KSSolverOptions())
...
function solve_operator(operator, u::AbstractVector, dt::Real, mesh::KSMesh)
    ...
    solution, _ = ProblemTypes.solve_problem(
        temp_problem,
        mesh,
        KSSolverOptions(max_iterations = 100, tolerance = 1e-6, adaptive = false)
    )
...
- The solution on the global domain.
"""
function solve_decomposed_problem(
        problem::AbstractKSProblem,
        mesh::KSMesh,
        num_subdomains::Int,
    ...
    return schwarz_method(
        problem,
        subdomains,
        interface_conditions,
        solver_options.max_iterations,
        solver_options.tolerance
    )
end
