...
"""
	h_refine(element::KSCell{T,N}) -> Vector{KSCell{T,N}}

Perform h-refinement on a given element by splitting it into smaller elements.

# Arguments
- `element::KSCell{T,N}`: The element to be refined.

# Returns
- `Vector{KSCell{T,N}}`: A vector of new elements resulting from the refinement.
"""
function h_refine(element::KSCell{T, N}) where {T <: Number, N}
    new_elements = KSCell{T, N}[]
    dimension = N
    sub_element_size = CommonMethods.element_size(element) ./ 2

    mid_points = [(node1 .+ node2) ./ 2 for node1 in element.points
                  for
                  node2 in element.points if node1 != node2]
    ...
    for i in 1:(2^dimension)
        ...
        diff_matrices = [SpectralMethods.derivative_matrix(getindex.(
                             new_collocation_points, j)) for
                         j in 1:dimension]

        new_element = KSCell{T, N}(
...
"""
	p_refine(element::KSCell{T,N}) -> KSCell{T,N}
...
# Returns
- `KSCell{T,N}`: The element with increased polynomial degree.
"""
function p_refine(element::KSCell{T, N}) where {T <: Number, N}
    ...
    new_diff_matrices = [SpectralMethods.derivative_matrix(getindex.(
                             new_collocation_points, j)) for
                         j in 1:dimension]
...
"""
	hp_refine(element::KSCell{T,N}, error::T, smoothness::T, threshold::T) -> Vector{KSCell{T,N}}
...
"""
function select_subelement_nodes(
        nodes::Vector{SVector{N, T}},
        mid_points::Vector{SVector{N, T}},
        index::Int
) where {T <: Number, N}
    ...
    for node in nodes
        if all(
            node[i] >= mid_points[i]
        for
        i in 1:length(mid_points) if (index - 1) & (1 << (i - 1)) != 0
        )
...
"""
	hp_refine_superposition(element::KSCell{T,N}, error::T, smoothness::T, threshold::T) -> Vector{KSCell{T,N}}
...
- `Vector{KSCell{T,N}}`: A vector of new elements resulting from the refinement.
"""
function hp_refine_superposition(
        element::KSCell{T, N},
        error::T,
        smoothness::T,
        threshold::T
...
"""
	superconvergence_refinement(element::KSCell{T,N}, solution::Vector{T}, problem::AbstractKSProblem) -> Vector{KSCell{T,N}}
...
- `Vector{KSCell{T,N}}`: A vector of new elements resulting from the refinement.
"""
function superconvergence_refinement(
        element::KSCell{T, N},
        solution::Vector{T},
...
"""
function adapt_mesh_superposition(
        mesh::KSMesh{T, N},
        solution::Vector{T},
        problem::AbstractKSProblem,
        threshold::T
    ...
    new_mesh = KSMesh{T, N}(
        ...
        tensor_product_masks = [trues(tuple(fill(el.polynomial_degree[1] + 1, N)...))
                                for el in new_elements],
        location_matrices = [Dict{Int, Int}() for _ in 1:length(new_elements)],
        standard_elements = Dict{Tuple{Int, Int}, StandardKSCell{T, N}}(),  # Assuming standard_elements for new mesh
...
"""
	estimate_error_and_smoothness(element::KSCell{T,N}, solution::Vector{T}, problem::AbstractKSProblem) -> Tuple{T,T}

Estimate the error and smoothness for a given element.

# Arguments
- `element::KSCell{T,N}`: The element to estimate error and smoothness for.
- `solution::Vector{T}`: The current solution vector.
- `problem::AbstractKSProblem`: The problem being solved.
...
"""
function estimate_error_and_smoothness(
        element::KSCell{T, N},
        solution::Vector{T},
        problem::AbstractKSProblem
) where {T <: Number, N}
...
"""
	compute_error_indicator(element::KSCell{T,N}, solution::Vector{T}, problem::AbstractKSProblem) -> Tuple{T,T}
