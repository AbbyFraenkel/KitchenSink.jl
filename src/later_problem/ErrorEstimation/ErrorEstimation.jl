...
export estimate_error,
       compute_error_indicator, compute_smoothness_indicator, compute_residual
...
"""
	estimate_error(element::KSCell{T,N}, solution::AbstractArray{T}, problem::KSProblem{T,N})::T where {T<:Real,N}

Estimate the error for a given element using a residual-based error estimator.

# Arguments
- `element::KSCell{T,N}`: The element to estimate the error for.
...
"""
	compute_residual(element::KSCell{T,N}, solution::AbstractArray{T}, problem::KSProblem{T,N})::AbstractArray{T} where {T<:Real,N}

Compute the residual for a given element.

# Arguments
- `element::KSCell{T,N}`: The element to compute the residual for.
- `solution::AbstractArray{T}`: The current solution vector.
- `problem::KSProblem{T,N}`: The problem being solved.
...
function compute_residual(
        ...
        solution::AbstractArray{T},
        problem::KSProblem{T, N}
)::AbstractArray{T} where {T <: Real, N}
...
"""
	compute_error_indicator(element::KSCell{T,N}, solution::AbstractArray{T}, problem::KSProblem{T,N})::Tuple{T,T} where {T<:Real,N}

Compute an error indicator for hp-adaptive refinement.

# Arguments
- `element::KSCell{T,N}`: The element to compute the error indicator for.
- `solution::AbstractArray{T}`: The current solution vector.
- `problem::KSProblem{T,N}`: The problem being solved.
...
"""
function compute_error_indicator(
        element::KSCell{T, N},
        solution::AbstractArray{T},
        problem::KSProblem{T, N}
)::Tuple{T, T} where {T <: Real, N}
...
"""
	compute_smoothness_indicator(element::KSCell{T, N}, solution::AbstractArray{S})::T where {T <: Real, S <: Real, N}

Compute the smoothness indicator for a given element and solution.

# Arguments
- `element::KSCell{T,N}`: The element to compute the smoothness indicator for.
...
function compute_smoothness_indicator(
        element::KSCell{T, N},
        solution::AbstractArray{S}
)::T where {T <: Real, S <: Real, N}
    coeffs = compute_expansion_coefficients(element, solution)
    return -log(norm(coeffs[(end - 1):end])) / sum(element.polynomial_degree)
end

"""
	apply_differential_operator(u::AbstractArray{T}, element::KSCell{T,N}, problem::KSProblem{T,N})::AbstractArray{T} where {T<:Real,N}
...
- `element::KSCell{T,N}`: The element to which the operator is applied.
...
"""
function apply_differential_operator(
        u::AbstractArray{T},
        element::KSCell{T, N},
        problem::KSProblem{T, N}
)::AbstractArray{T} where {T <: Real, N}
    ...
    for i in eachindex(u)
        ...
        result[i] = problem.equation(
            element.collocation_points[i].coordinates..., derivatives...)
    end
...
"""
	compute_expansion_coefficients(element::KSCell{T, N}, solution::Vector{T}; lambda::T = 1e-6) where {T <: Real}

Compute the expansion coefficients for the solution on the given element.

# Arguments
- `element::KSCell{T,N}`: The `KSCell` object on which to compute the expansion coefficients.
...
"""
function compute_expansion_coefficients(
        element::KSCell{T, N},
        solution::Vector{T};
        lambda::T = 1e-6
) where {T <: Real, N}
    ...
    if length(solution) != length(element.collocation_points)
        throw(
            DimensionMismatch(
            "Solution length does not match the number of collocation points",
        ),
        )
    end

    # Retrieve the standard element for this element's polynomial degree
    std_elem = SpectralMethods.get_or_create_standard_cell(element.polynomial_degree)
    ...
    for i in eachindex(basis_functions)
        for j in eachindex(basis_functions)
            A[i, j] = sum(
                basis_functions[i].function_handle(p.coordinates) *
                basis_functions[j].function_handle(p.coordinates)
            for
            p in collocation_points
            )
        end
        b[i] = sum(
            solution[k] *
            basis_functions[i].function_handle(collocation_points[k].coordinates)
        for
        k in 1:length(solution)
        )
    end
...
"""
	estimate_smoothness(element::KSCell{T, N}, solution::AbstractVector{T}) where {T <: Real, N}

Estimate the smoothness of the solution on the given element.

# Arguments
- `element`: The `KSCell` object on which to estimate smoothness.
- `solution`: The solution vector.

# Returns
- A scalar value representing the estimated smoothness.
"""
function estimate_smoothness(
        element::KSCell{T, N},
...
"""
function estimate_multilevel_error(
        mesh::KSMesh{T, N},
        solution::AbstractArray{T},
        problem::KSProblem{T, N}
)::AbstractArray{T} where {T <: Real, N}
    error_estimates = similar(solution, length(mesh.cells))
...
"""
function compute_multilevel_error_indicator(
        mesh::KSMesh{T, N},
        solution::AbstractArray{T},
        problem::KSProblem{T, N}
)::AbstractArray{Tuple{T, T}} where {T <: Real, N}
    error_indicators = Array{Tuple{T, T}}(undef, length(mesh.cells))
